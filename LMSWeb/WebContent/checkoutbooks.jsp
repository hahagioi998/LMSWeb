...
<div class="container">
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination">
			...
			<li class="page-item"><a class="page-link" href="checkOut?pageNo=<%=i%>&cardNo=<%=cardNo%>&branchId=<%=branchId%>"><%=i%></a></li>
			<%} %>
			<li class="page-item"><a class="page-link" href="#"
	...
	<table class="table table-striped">
		...
		<tr>
			...
			<td>
				<%
					for(BookCopies bc: b.getBookCopies()){
						if(bc.getBranchId()==branchId){
							noOfCopies = bc.getNoOfCopies();
			...
			<td><button type="button"
					onclick="javascript:location.href='checkBookOut?bookId=<%=b.getBookId()%>&cardNo=<%=cardNo%>&branchId=<%=branchId%>'"
					class="btn btn-primary btn-sm"
					<%if(noOfCopies<=0){ %>disabled<%} %>>CheckOut</button></td>
		</tr>
		<%
			}
		%>
